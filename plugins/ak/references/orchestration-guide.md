...
4. Create a run and immutable manifest/source snapshot.
5. Create module-aware task envelopes from `orchestration/waves.json` and `orchestration/roles.json`.
6. Dispatch only tasks whose dependencies passed. Leaf modules may fan out to separate workers; parent/cross-module synthesis follows their handoffs.
7. Validate every handoff before advancing the wave.
8. Merge evidence deterministically and preserve conflicts.
9. Publish phases in order and pause at configured human checkpoints.
10. Run independent QA before derived rendering.
